...
_PAGE_ID = os.getenv("NOTION_PAGE_ID")
...
def _get_client():
    """Internal: Get or create the Notion client instance."""
    global _notion
    if _notion is None:
...
def _get_headers():
    """Internal: Get headers for direct API requests."""
    return {
...
def write_page(text):
    ...
    try:
        ...
        raise Exception(f"Error writing to Notion page: {str(e)}")
...
def ReadTasks(filter_date=None, sort_by_priority=False):
    ...
    try:
        ...
        raise Exception(f"Error reading tasks from database: {str(e)}")
...
def AddTask(task, due_date=None, priority=None):
    ...
    try:
        ...
        raise Exception(f"Error adding task to database: {str(e)}")
...
def DeleteTask(task_id):
    ...
    try:
        ...
        raise Exception(f"Error deleting task from database: {str(e)}")
